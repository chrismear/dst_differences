...
end_time = Time.now.to_time + 5*365*24*60*60

dst_entrances1 = tz1.transitions_up_to(end_time).select { |transition|
...
dst_entrances2 = tz2.transitions_up_to(end_time).select { |transition|
  transition.offset.std_offset == 3600 &&
    transition.previous_offset.std_offset == 0
}
dst_entrance_days2 = dst_entrances2.map { |transition| transition.local_end_at.to_time }

dst_exits1 = tz1.transitions_up_to(end_time).select { |transition|
...
dst_exits2 = tz2.transitions_up_to(end_time).select { |transition|
  transition.offset.std_offset == 0 &&
    transition.previous_offset.std_offset == 3600
}
